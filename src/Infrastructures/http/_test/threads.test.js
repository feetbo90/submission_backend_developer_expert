...
describe('/threads endpoint', () => {
  ...
  describe('when POST /threads', () => {
    ...
    it('should response 400 when title more than 255 characters', async () => {
      // Arrange
      const requestPayload = { title: 'Thread Judul'.repeat(256), body: 'Thread Content' };
      const server = await createServer(container);
      ...
      expect(response.statusCode).toEqual(400);
      expect(responseJson.status).toEqual('fail');
      expect(responseJson.message).toEqual('tidak dapat membuat thread karena karakter title melewati batas limit');
    });
  });
